# Summary-level simulations with directional pleiotropy and InSIDE assumption satisfied
rm(list = ls())
library(data.table)
library(dplyr)
library(MASS)
...
sigma2x = sigma2y = 5e-5; sigma2x_td = sigma2y_td = 5e-5
...
est = matrix(NA, nrow = 200, ncol = 3+11*3)
mr_methods = c("IVW", "median", "mode", "PRESSO", "Robust", "Lasso", "egger", "conmix", "MRMix", "RAPS", "WWER")
nmeth = length(mr_methods)
...
for (repind in 1:200){
    ...
    betahat_x_1 = betax + rnorm(M, mean = 0, sd = sqrt(1/nx))
    betahat_y_1 = betay + rnorm(M, mean = 0, sd = sqrt(1/ny))
    betahat_x_2 = betax + rnorm(M, mean = 0, sd = sqrt(1/nx))
    betahat_y_2 = betay + rnorm(M, mean = 0, sd = sqrt(1/ny))

    # Filter the SNPs that reach genome-wide significance in the study associated with X.
    ind_filter = which(2*pnorm(-sqrt(nx)*abs(betahat_x_1))<pthr)
    ...
    if (numIV>2){
        betahat_x.flt = betahat_x_2[ind_filter]
        betahat_y.flt = betahat_y_2[ind_filter]
        ...
        est[repind,3+1+nmeth] = res$StdError
        est[repind,3+1+2*nmeth] = T1-T0
        rm(res)
        ...
        est[repind,3+2+nmeth] = res$StdError
        est[repind,3+2+2*nmeth] = T1-T0
        rm(res)
        # 3. mode
        T0 = proc.time()[3]
        res = mr_mbe(mr.obj)
        ...
        est[repind,3+3+nmeth] = res$StdError
        est[repind,3+3+2*nmeth] = T1-T0
        rm(res)
        # 4.MR-PRESSO
        ...
        if (N<=2e5){
            ...
            if (!is.na(res$`Main MR results`[2,"Causal Estimate"]) & !is.na(res$`Main MR results`[2,"Sd"])){
                ...
                est[repind,3+4+nmeth] = res$`Main MR results`[2,"Sd"]
            } else{
                est[repind,3+4] = res$`Main MR results`[1,"Causal Estimate"]
                est[repind,3+4+nmeth] = res$`Main MR results`[1,"Sd"]
            }
            est[repind,3+4+2*nmeth] = T1-T0
            rm(res)
        }
        # 5. robust
        T0 = proc.time()[3]
        ...
        est[repind,3+5+nmeth] = res$StdError
        est[repind,3+5+2*nmeth] = T1-T0
        rm(res)
        # 6. MR-Lasso
        T0 = proc.time()[3]
        ...
        est[repind,3+6+nmeth] = res$ThetaSE
        est[repind,3+6+2*nmeth] = T1-T0
        rm(res)
        # 7. Egger
        T0 = proc.time()[3]
        res = mr_egger(mr.obj)
        ...
        est[repind,3+7+nmeth] = res$StdError.Est
        est[repind,3+7+2*nmeth] = T1-T0
        rm(res)
        ...
        est[repind,3+8+nmeth] = sum(CIlength)/1.96/2 ## Caution: this may be problematic
        est[repind,3+8+2*nmeth] = T1-T0
        rm(res)
        # 9. MRMix
        ...
        est[repind,3+9+nmeth] = res$SE_theta
        est[repind,3+9+2*nmeth] = T1-T0
        rm(res)
        # 10. MR-RAPS
        T0 = proc.time()[3]
        ...
        est[repind,3+10+nmeth] = res$beta.se
        est[repind,3+10+2*nmeth] = T1-T0
        rm(res)
        # 11. WWER
        sumstats_select <- list(beta_hat = as.data.frame(list(betahat_x_1, betahat_y_1), col.names = c("X", "Y")),
                                se_hat = as.data.frame(list(rep(1/sqrt(nx), M), rep(1/sqrt(ny), M)), col.names = c("X", "Y")))
        sumstats_fit <- list(beta_hat = as.data.frame(list(betahat_x_2, betahat_y_2), col.names = c("X", "Y")),
                             se_hat = as.data.frame(list(rep(1/sqrt(nx), M), rep(1/sqrt(ny), M)), col.names = c("X", "Y")))
        selected <- WWER::select_snps(sumstats_select)
        T0 = proc.time()[3]
        res <- WWER::fit_tce(sumstats_fit, selected)
        T1 = proc.time()[3]
        est[repind, 3+11] = res$R_tce[1, 2]
        est[repind, 3+11+nmeth] = res$SE_tce[1, 2]
        est[repind, 3+11+2*nmeth] = T1-T0
        rm(res)
    }
    if (repind%%5==0){
        print(paste("Rep",repind,"numIV",numIV))
        save(est, file = paste0("results/dir_IA_", "est_theta",theta,"_thetaU",thetaU,"_N",N,"_prop_invalid",prop_invalid,".rda"))
    }
}
save(est, file = paste0("results/dir_IA_", "est_theta",theta,"_thetaU",thetaU,"_N",N,"_prop_invalid",prop_invalid,".rda"))
